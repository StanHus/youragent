...
echo ""
echo "${BOLD}${CYAN}OpenClaw detected!${RESET}"
echo ""
echo "Configure your OpenClaw agents to auto-read .agent/ folders?"
...
echo ""
read -p "Configure OpenClaw agents? (y/n): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
## Working in Code Repositories (YourAgent Integration)
...
- **Global identity**: `~/your-workspace/IDENTITY.md` (who you are)
- **Global memory**: `~/your-workspace/memory/` (your accumulated knowledge)
...
You:
1. Load global identity from ~/your-workspace/IDENTITY.md
2. Load global memory from ~/your-workspace/memory/
...
while IFS='|' read -r workspace name; do
  ...
  if grep -q "$INTEGRATION_MARKER" "$AGENTS_MD"; then
  ...
  echo "$INTEGRATION_SNIPPET" >> "$AGENTS_MD"
